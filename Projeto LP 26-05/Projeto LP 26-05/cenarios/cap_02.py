...
def iniciar(nome_char, classe, status_list, nome_dog):
    ...
    print(f'Ao proceguir pela trilha, {nome_char} se depara com uma ponte feita de corda e madeira, que passa por cima de um precipício, o outro caminho mais seguro \nlevaria dias para ser alcançado, então {nome_char} decide atravessar a ponte. Mesmo com o vento forte e a condição precária da ponte, {nome_char} está \npraticamente conseguindo atravessá-la, até que escuta um estalo, e ao olhar pra trás {nome_char}, percebe que {txt_yellow}a ponte está prestes a cair!{end_txt}\n')

    cen2_arq = ''

    while True:

        print(f'{txt_green}O que você decide fazer?{end_txt} \n 1 - Correr para cruzar a ponte antes que ela caia \n 2 - Reunir poder mágico para cruzar o restante do caminho independente da ponte \n 3 - Segurar as cordas laterais e andar com cuidado \n 4 - Tentar voltar. Seguir parece não ser uma alternativa. {nome_char} tenta voltar ao início da ponte para usar o caminho mais seguro \n')
        ...
        if cen2_escolha != 0 and cen2_escolha != 4:
            if 'machucado' in status_list:
                sucesso = projeto_funções.dados_cen2_machucado()
            else:
                sucesso = projeto_funções.dados_cen2()
        ...
        match cen2_escolha:
            ...
            case 1: # Correr
                if 'machucado' in status_list:
                    if sucesso:
                        ...
                        cen2_arq = f'Por estar machucado(a), {nome_char} nao conseguiu cruzar a ponte a tempo, e caiu para sua morte\n'
                        projeto_arquivos.salvar(cen2_arq)
                    ...
                    if sucesso:
                        ...
                        cen2_arq = f'Correu da ponte que caisse e conseguiu cruza-la\n'
                        projeto_arquivos.salvar(cen2_arq)
                        break
                    else:
                        print(f'{txt_red}{nome_char} corre com todas as suas forças mas mesmo assim não consegue cruzar a ponte antes que a mesma caia...{end_txt}')
                        cen2_arq = f'Tentou correr para cruzar a ponte, mas nao conseguiu e caiu pra sua morte\n'
                        projeto_arquivos.salvar(cen2_arq)
                        projeto_funções.fim_de_jogo()
            ...
            case 2: # Magia
                if classe == 3:
                    if sucesso:
                        ...
                        cen2_arq = f'{nome_char} concentrou as suas forças arcanas e conjurou um feitico de voo, e cruzou a ponte voando antes que a mesma caisse.\n'
                        projeto_arquivos.salvar(cen2_arq)
                        break
                    else:
                        print(f'{txt_red}{nome_char} tentou se concentrar em um feitiço de vôo, mas falhou e caiu{end_txt}')
                        cen2_arq = f'{nome_char} tentou se concentrar em um feitico de voo para cruzar a ponte que caisse, mas falhou e caiu no penhasco\n'
                        projeto_arquivos.salvar(cen2_arq)
                        projeto_funções.fim_de_jogo()
                        break
                    ...
                    if sucesso:
                        ...
                        cen2_arq = f'{nome_char} chamou a natureza e conjurou raizes para sustentar a ponte que caisse, dando tempo o suficiente para atravessar a mesma\n'
                        projeto_arquivos.salvar(cen2_arq)
                        break
                    else:
                    ...
                    cen2_arq = f'{nome_char} tentou usar magia mesmo nao tendo escolhido uma classe que possui essas habilidades, e caiu da ponte no penhasco\n'
                    projeto_arquivos.salvar(cen2_arq)
                    projeto_funções.fim_de_jogo()
                    break
            ...
            case 3: # Andar com cuidado
                if sucesso:
                    print(f'{txt_green}{nome_char} segurou firme nas cordas, andou devagar e com cuidado, e conseguiu atravessar a ponte{end_txt}')
                    cen2_arq = f'{nome_char} atravessou a ponte com cuidado e paciencia\n'
                    projeto_arquivos.salvar(cen2_arq)
                    break
                else:
                    print(f'{txt_red}{nome_char} andou lentamente, mas a ponte caiu antes de {nome_char} conseguir atravessar{end_txt}')
